...
- Successful V1 responses print `data` to stdout. Standard API errors (including 401) print the server's `error` unchanged to stderr and exit nonzero.
- Other V1 runtime errors print JSON `{code,message,details?}` to stderr: `HTTP_<status>` for nonstandard HTTP errors, `NETWORK_ERROR` for connection failures, `REQUEST_TIMEOUT` for HTTP timeouts, and `CLI_ERROR` for unexpected exceptions. A nonstandard 401 uses `UNAUTHORIZED` with `details.login_url`. Local token/argument errors and help/save-token messages keep their existing text format; stderr is not universally JSON.
